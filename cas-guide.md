...
`cas_score` is the base weighted sum (`CAS.Overall`). For corroboration bonus and `effective_score`, use `atb verify --json` (diagnostic mode). See [`api/verify-schema.md`](api/verify-schema.md).
