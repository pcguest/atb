...
func TestBuildReportsMalformedAndUnavailableContextReferences(t *testing.T) {
	...
	records := []bundle.Record{
		...
		{Event: hash.Event{Sequence: 4, Type: "ai.context.operation", Data: map[string]any{"operation_id": "negative-tokens", "operation": "select", "input_unit_ids": []any{}, "output_unit_ids": []any{}, "input_token_count": -1, "output_token_count": -0.5}}},
	}
	lineage := Build(records)
	if len(lineage.Operations) != 1 || lineage.Operations[0].InputTokenCount != nil {
		t.Fatalf("operations = %#v", lineage.Operations)
	}
	if len(lineage.Warnings) != 5 {
		t.Fatalf("warnings = %#v, want malformed unit, missing parent, malformed operation, and two negative token counts", lineage.Warnings)
	}
	...
	if !containsWarning(lineage.Warnings, "output_token_count must not be negative") {
		t.Fatalf("negative fractional token warning missing: %#v", lineage.Warnings)
	}
}
