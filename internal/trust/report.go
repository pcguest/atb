...
func cryptoCategory(bundlePath string, repoRoot string) Category {
	checks := []Check{
		evidenceCheck(
			...
			"RFC8785-compatible canonicalization spec is missing (expected docs/specification/bundle-v1.md).",
			SeverityAdvisory,
			false,
		),
	}
...
func operationalSafetyCategory(repoRoot string) Category {
	checks := []Check{
		...
		evidenceCheck(
			...
			"Incident response runbook is missing (expected SECURITY.md).",
			SeverityAdvisory,
			false,
		),
		evidenceCheck(
			...
			"Operational security guidance is missing (expected docs/concepts/trust-model.md).",
			SeverityAdvisory,
			false,
		),
	}
...
func documentationCategory(repoRoot string) Category {
	checks := []Check{
		evidenceCheck(
			...
			"Quickstart guide is missing (expected docs/getting-started/quickstart.md).",
			SeverityAdvisory,
			false,
		),
		evidenceCheck(
			...
			repoRoot,
			"docs/maintainers/automation-contract.md",
			"ATB includes the AI integration contract documentation.",
			"AI integration contract doc is missing (expected docs/maintainers/automation-contract.md).",
			SeverityAdvisory,
			false,
		),
		evidenceCheck(
