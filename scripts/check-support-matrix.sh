...
expect "go.mod Go version" "$GO_MOD_VERSION" "1.26.3"
expect "Makefile GOTOOLCHAIN" "$MAKEFILE_TOOLCHAIN" "go1.26.3"

while IFS= read -r wf; do
  if grep -q "setup-go@" "$wf"; then
    if grep -q 'go-version:' "$wf" && ! grep -q 'go-version: "1.26.3"' "$wf"; then
...
require_file_contains "docs/support-matrix.md" "Go 1\\.26\\.3" "Go support matrix entry"
