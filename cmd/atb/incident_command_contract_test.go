...
func TestRunIncidentListAndExportArgumentContracts(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "export endpoint value", args: []string{"export", "--custos-endpoint"}, want: "missing value for --custos-endpoint"},
		...
		{name: "export session required", args: []string{"export", "--bundle=" + bundlePath, "--out=pack.zip"}, want: "--session is required"},
		{
			...
			args: []string{"export", "--bundle=" + bundlePath, "--session=sess-A", "--out=pack.zip", "--custos-endpoint=https://example.test"},
...
func TestRunIncidentExportToCustodyEndpoint(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = w.Write([]byte(`{"receipt_version":"mortise.receipt.v1","receipt_id":"receipt-1","bundle_hash":"sha256:bundle"}`))
	}))
	...
	code = runIncident([]string{
		...
		"--bundle=" + bundlePath,
		"--session=sess-A",
		"--custos-endpoint=" + failServer.URL,
	}, &stdout, &stderr)
	if code != exitSystemError || !strings.Contains(stderr.String(), "push to Custos") {
