...
for capture; all other headers are omitted. See the
[trust model](../concepts/trust-model.md#data-classification).
...
> **No live traffic or provider credentials?** Run the complete application
> boundary demo:
>
> ```bash
> make demo-incident
...
```bash
atb sign --bundle run.atb/incident-demo/incident.atb --key atb-key.pem
...
See [Quickstart](../getting-started/quickstart.md) for a full policy-decision
example.
