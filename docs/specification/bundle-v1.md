...
- Each NDJSON record must fit on a single line of at most **16,777,216 bytes**
  (16 MiB, `MaxLineSizeBytes`).
- The Go CLI and the Python and TypeScript SDK readers default to a total input
  limit of **536,870,912 bytes** (512 MiB) and **1,000,000 non-empty records**.
  SDK callers may choose lower positive limits. Inputs above the defaults may
  be valid v1 bundles but are rejected by current readers as unsafe to load in
  memory.
...
These types are defined for backward compatibility with bundles created before the current AI trace specification. New integrations MUST use the types defined in `docs/specification/events.md` instead.
...
|------|-------------|-----------|
| `ai.llm.call` | Canonical LLM lifecycle event type for new integrations | `docs/specification/events.md` |
| `ai.tool.exec` | Canonical tool execution event type for new integrations | `docs/specification/events.md` |
| `ai.chain.run` | Canonical chain or step lifecycle event type for new integrations | `docs/specification/events.md` |
...
- `atb.profile.data_export` requires `atb.bundle.manifest`, `ai.request.received`, `ai.policy.decision`, `data.export.precommit`, and `data.export.executed` (`internal/profiles/templates/data_export.yaml`). `ai.human.approval` is warning-level evidence and is evaluated when exports execute (`required_when` `data.export.executed`). `ai.action.*` is a distinct privileged-action family and is not this profile's critical path.
...
Push transport behaviour is documented separately in
[`bundle-push.md`](./bundle-push.md). That document is outside the frozen v1.0
