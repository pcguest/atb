...
The following records explain earlier retry and provenance safeguards. They are
not the current release status; check the local public-readiness report and all
registries before choosing a new version.
...
immutable once published, and a later signing or upload failure may require
rerunning the job.
...
serialization. The retained historical artifact remains part of the release history; a
